...
import { BuiltinTypeKind } from "../types/BuiltinType";
...
export function extract(checker: ts.TypeChecker, type: ts.Type) {
    ...
    while (stack.length > 0) {
        ...
        if (currentType.type === "union" || currentType.type === "intersection") {
            for (const includedType of (current.type as ts.UnionOrIntersectionType).types) {
                stack.push({
                    type: includedType,
                    base: currentType,
                });
...
function newType(checker: ts.TypeChecker, tsType: ts.Type): Type {
    ...
    if (isDateType(checker, tsType)) {
        return {
            type: "builtin",
            str,
            kind: BuiltinTypeKind.Date,
        };
    }
...
function isDateType(checker: ts.TypeChecker, tsType: ts.Type) {
    // TODO [RM]: super-hacky way to detect Date; figure out better way.
    if (!isObjectType(tsType)) {
        return false;
    }

    const typeName = checker.typeToString(tsType);
    if (typeName !== "Date") {
        return false;
    }

    const props = getProperties(checker, tsType);
    for (const propName of ["getFullYear", "getUTCDay", "setUTCHours", "toISOString"]) {
        if (!props.some(q => q.name === propName)) {
            return false;
        }
    }
...
function getBasicTypeKind(tsType: ts.Type): BasicTypeKind | undefined {
    ...
    if (hasFlag(tsType, ts.TypeFlags.ESSymbol) || hasFlag(tsType, ts.TypeFlags.UniqueESSymbol)) {
        return BasicTypeKind.Symbol;
    }

    if (hasFlag(tsType, ts.TypeFlags.Undefined) || hasFlag(tsType, ts.TypeFlags.Void)) {
...
function getLiteralValue(tsType: ts.Type): LiteralValue {
    if (hasFlag(tsType, ts.TypeFlags.BooleanLiteral)) {
        ...
        throw new Error(`Unsupported tsType.intrinsicName = '${(tsType as any).intrinsicName}'.`);
    }
